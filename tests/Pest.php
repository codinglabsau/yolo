...
/**
 * Bind a mock SSM client with command-routed responses, capturing every call.
 * A command's value may be a single Result (repeated) or an array of Results
 * used as a queue (the last entry repeats once exhausted). Mirrors
 * bindMockEc2Client.
 *
 * @param  array<string, Result|array<int, Result>>  $byCommand
 * @param  array<int, array{name: string, args: array<string, mixed>}>  $captured
 */
function bindMockSsmClient(array $byCommand, array &$captured): void
{
    $mock = new class($byCommand, $captured) extends MockHandler
    {
        /** @var array<string, int> */
    ...
    Helpers::app()->instance('ssm', new SsmClient([
        'region' => 'ap-southeast-2',
        'version' => 'latest',
        'credentials' => false,
        'handler' => $mock,
...
/**
 * Bind a mock Route 53 client with command-routed responses, capturing every
 * call. A command's value may be a single Result (repeated) or an array of
 * Results used as a queue (the last entry repeats once exhausted). Mirrors
 * bindMockEc2Client. (bindMockRoute53Client in SyncsRecordSetsTest.php is
 * zone-list specific; this one routes any command.)
 *
 * @param  array<string, Result|array<int, Result>>  $byCommand
 * @param  array<int, array{name: string, args: array<string, mixed>}>  $captured
 */
function bindRoutedRoute53Client(array $byCommand, array &$captured): void
{
    $mock = new class($byCommand, $captured) extends MockHandler
    {
        /** @var array<string, int> */
