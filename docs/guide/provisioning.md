...
|---|---|---|---|
| `yolo sync:account <env>` | **Account** | the whole AWS account | the ECS / Application Auto Scaling / ElastiCache service-linked roles (created if missing — a greenfield account can't create its first cluster or scalable target without them), GitHub OIDC provider, the `yolo-users` self-service group ([Developer Credentials](/guide/credentials#who-can-do-what)) — the group and its policy only; membership comes with the user, granted by [`yolo permissions`](/reference/commands#yolo-permissions) |
...
| `yolo sync:app <env>` | **App** | one app | S3 buckets, app IAM (deployer role/policy, the per-app observer role/policy, the per-app data-read (when the app declares a [`bucket`](/reference/manifest#bucket)) / data-write policies fenced to this app's data bucket and dump prefix, the per-app developer role, the `yolo-{env}-{app}-observers` / `-developers` grant groups, the per-app ECS task role + any [`task-role-policies`](/reference/manifest#task-role-policies)), ECS cluster/service/task definition, target group + listener rule, CloudFront distribution, hosted zone & ACM certificate, SQS queues, CloudWatch dashboard — plus the shared [Valkey cache](#cache-and-sessions) (default-on for any app with tasks; opt out via `cache.store`). Sessions ride the same Valkey cluster by default, so they need no provisioning of their own |
...
- **`apps/{app}.yml`** — each app's published claim (the app's name, the [`services`](/reference/manifest#services) it uses, and its [`bucket`](/reference/manifest#bucket)). Only `sync:app` writes it — an admin act, never a deploy — so the environment always knows which apps use which shared services and which bring-your-own buckets the data tiers must reach; a claim that lags the manifest is drift the deploy gate refuses on.
