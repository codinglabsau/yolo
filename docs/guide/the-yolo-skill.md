...
That read-first posture is also enforced below the convention line. YOLO's read commands (`status`, `audit` and friends) **mint a scoped token** by assuming the [`yolo-{env}-observer-role`](/guide/provisioning) — the developer still authenticates as themselves, but the command is capped to the read-only observer policy by construction, so it can't mutate even if the developer's own identity could. It's **fail-closed**: if the role can't be assumed the command refuses rather than falling back to the developer's full identity. The single deliberate escape is `--dangerously-skip-permissions`, which skips the cap (with a loud warning) for bootstrapping a fresh environment or breaking glass.
