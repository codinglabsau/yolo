...
- A tiny supervised process in the web container reads FrankenPHP's metrics endpoint and **publishes the saturation directly via `PutMetricData`** — but only while it's hot (≥70%), and it snoozes the cooldown after a tripping datapoint (one breach already steps the count out), so CloudWatch is touched only during a spike. Going direct keeps the signal seconds-fresh; an EMF log line would have to traverse the logs pipeline (~30–90s) before surfacing as a metric — too slow for the spike burst exists to catch. The cost is one namespace-scoped `cloudwatch:PutMetricData` grant on the task role plus the `aws/aws-sdk-php` the app almost always already ships (a build preflight hard-fails if it doesn't).
...
- Cost is dominated by the one high-resolution alarm — roughly **$0.30/month** per app; the metric puts are near-zero because nothing is published until the service is hot, and then only at the cooldown cadence.
