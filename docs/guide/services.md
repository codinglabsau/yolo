...
- **`yolo:search:heal`** — schedule it (`$schedule->command('yolo:search:heal')->everyFiveMinutes();`). It walks the app's searchable models — the `scout.typesense.model-settings` keys, unioned with a sweep of `app/` for anything using Scout's `Searchable` (resolved through wrapper traits too) — and when a model's collection is missing, or sits empty while the database plainly isn't, it queues that model's rebuild. A full cluster loss costs one heal interval plus a reimport, unattended. Self-guarded with a cache lock (on combined-services apps the scheduler fires on every web task — one wipe must mean one rebuild), and it deliberately doesn't chase partial drift: live churn makes count-matching a false-alarm machine. A cluster that stops honouring the app's key is reported, not "healed" — that's `yolo sync:app`'s fix.
