...
**The index self-heals — persistence is a recovery loop, not a disk.** YOLO ships two artisan commands into every consuming app (registered by the runtime service provider; the app's `TYPESENSE_*`/Scout config is all they need):

- **`yolo:search:heal`** — schedule it (`$schedule->command('yolo:search:heal')->everyFiveMinutes();`). It walks the app's searchable models — the `scout.typesense.model-settings` keys, unioned with a sweep of `app/` for anything using Scout's `Searchable` (resolved through wrapper traits too) — and when a model's collection is missing, or sits empty while the database plainly isn't, it queues that model's rebuild. A full cluster loss costs one heal interval plus a reimport, unattended. Run-once is the command's own property — it takes a cache lock itself, so it needs no `onOneServer()`/`withoutOverlapping()` decorations (though adding them is harmless): on combined-services apps the scheduler fires on every web task, and the built-in lock also covers a heal pass outlasting the next tick and a manual run racing the scheduled one. It deliberately doesn't chase partial drift: live churn makes count-matching a false-alarm machine. A cluster that stops honouring the app's key is reported, not "healed" — that's `yolo sync:app`'s fix.
- **`yolo:search:reimport {model?*}`** — `scout:import --fresh` without the blackout. Each collection is rebuilt beside the live one and swapped in atomically via a Typesense **collection alias**: build a timestamped collection from the model's declared schema, import everything into it while the live index keeps serving, flip the alias, delete the old collection, then **replay rows that changed during the build window** through Scout's normal path (`updated_at >=` build start — the database is the source of truth, so catch-up is never guesswork). Exact mirror (orphans die with the old collection), current schema applied, zero search downtime. The first run per model migrates Scout's literal collection to the alias layout (a one-time sub-second gap); a record *deleted* mid-build lingers until the next reimport — the temporary-index pattern's standard blind spot. During each swap the old and new collections coexist, so run order is smallest-first (the command does this itself) and rebuilding a large collection on tightly-sized nodes may want `services.typesense.memory` bumped first.
