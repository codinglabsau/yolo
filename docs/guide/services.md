...
**Keys are scoped per app.** The cluster admin key lives alone in the env-shared `.env` and never leaves the env tier (baked into the env-scoped image; app builds never read it). `sync:app` mints each consuming app a key restricted to its own `{prefix}*` collections via the admin API — a leaked app key reaches that app's collections only — and writes it into the app's **environment-side `.env`** (`env/.env.{app}` in the env config bucket), beside the env manifest and the per-app claim files. Keeping it there, not in the app's developer `.env` (a per-app config bucket the admin tier running `sync` is fenced from), is what lets `sync` mint it at all — and it isolates each key: the app's build merges in only its own `env/.env.{app}`, never the admin key or a sibling's. While the cluster isn't up yet, the mint skips with instructions and lands on the next sync. Browser search keys (search-only, the Algolia secured-key model) are minted app-side. The build also injects `SCOUT_DRIVER=typesense`, `SCOUT_PREFIX`, and the private Cloud Map node addresses (`TYPESENSE_HOST/PORT/PROTOCOL` + the full `TYPESENSE_NODES` list).
