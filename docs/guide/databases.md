...
**In-place cutover** shrinks the window to the length of a loop over the running tasks, and is what the `scripts/db-cutover.sh` prototype does (Fargate-only; it graduates into a `yolo db:cutover` command in time): a maintenance page up on every task, then per task patch `.env`, `php artisan optimize`, and `octane:reload` (web) or `queue:restart` (queue) so the booted worker actually re-reads the change, then the page back down. `scripts/db-cutover-verify.sh` proves the result across four independent layers per container — the `.env` line, the *cached* config value the app really uses, a live query, and maintenance-mode-off — and then asserts every container reported the **same** `@@server_uuid`. That cross-container identity check is the one that matters: it catches a single straggler still talking to the old database even when every hostname reads clean.

Two things the scripts assume you've done, because they're the sharp edges:
