...
| Tier | Group | Grants |
|---|---|---|
| Observer — environment | `yolo-{env}-observers` | read every app in the environment, its data included (objects in every YOLO-named app data bucket; a bring-your-own bucket through that app's per-app tier) |
| Observer — one app | `yolo-{env}-{app}-observers` | read one app and its data bucket (log content fenced to its log group) |
| Developer — environment | `yolo-{env}-developers` | observer, plus **write** every app's data (objects in the YOLO-named data buckets) and read the [database dumps](/guide/databases#logical-backups) — no infrastructure writes, no `sync` |
| Developer — one app | `yolo-{env}-{app}-developers` | the same for one app: its data bucket and its own dump prefix |
| Admin — environment | `yolo-{env}-admins` | everything: `sync` / `scale` / `services` / `deploy` from a laptop / the environment's manifest and shared `.env` / manage access (**fresh MFA code per run**) |

The tiers split along two axes — **infrastructure** and **data**, each read or write — and a tier is a composition of them. The observer reads both; the developer adds data writes and nothing on the infrastructure axis; the admin adds the infrastructure writes. Data lives in its own policy documents (`yolo-{env}-data-read` / `-data-write`, with per-app twins) rather than on the observer document, because the per-app observer document is also what the CI deployer role carries for its pre-deploy drift check — a deploy grant must never be able to read user uploads. The env-wide documents cover the YOLO-named data buckets by wildcard only; a bring-your-own bucket is named in the app's published claim, which CI writes on every deploy, so it is deliberately never turned into an env-wide grant — reach it through that app's per-app tier (the env tiers can assume every per-app role). The developer tier deliberately excludes `yolo run` and `db:backup`: both run code in the app's task role, which reaches every secret the app holds, not just its data.

**There is no human deployer grant.** The per-app deployer role is CI's — assumed keylessly from GitHub Actions ([CI/CD](/guide/ci-cd)) — and an admin already assumes it for a laptop `deploy`. Developers ship through CI.
