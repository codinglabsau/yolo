...
Every sync plan also compares the running CLI's release against the environment's [version-of-record](/guide/provisioning#the-environment-declaration) (the `yolo-version` marker `sync:environment` stamps into the env config bucket) and **warns** when this checkout is provably older — an older CLI's checks predate the environment, so its "in sync" can be silence rather than truth. A warning only, never a refusal.
