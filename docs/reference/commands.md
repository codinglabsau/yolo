...
- Writes a default `Dockerfile`, `.dockerignore`, and the baseline `docker/php.ini` (asks before overwriting existing ones). The ini is [published once and then the app's to tune](/guide/images#php-configuration) — YOLO never rewrites it.
...
- **The env buckets go with the environment — except the dumps bucket.** The env config bucket (the env manifest + env-shared `.env`) and the env logs bucket (ALB access logs) are regeneratable infrastructure config, emptied and deleted as part of the teardown. The env dumps bucket (the apps' [database dumps](/guide/databases#logical-backups)) survives, like the app data bucket — a backup's entire purpose is to outlive the loss of everything else, so it's removed by hand once the data is provably no longer needed.
