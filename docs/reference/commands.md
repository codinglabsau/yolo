...
A per-app **CloudWatch dashboard** (`yolo-<env>-<app>-dashboard`) is generated last, so every resource it charts already exists. It panels each ECS service (web, plus any extracted queue/scheduler — CPU/memory/tasks/network each), the ALB (target health, requests, latency, slow-request bands, error counts and a 5xx error-rate SLO), SQS depth/throughput, the asset CloudFront distribution (requests, errors and cache hit rate), the S3 buckets, any consumed services (MediaConvert jobs, Rekognition requests) and the app's logs — plus an RDS panel sourced from the manifest [`database:`](/reference/manifest#database) key, omitted when it isn't set (CPU, connections, memory, throughput and read/write latency). The RDS panel reads the manifest, never the app's secret `.env`. It's a read-only convenience: CloudWatch dashboards can't carry tags, so it doesn't appear in `yolo audit`.
...
```bash
yolo destroy:environment <environment> [--delete-data] [--check] [--force] [--no-progress]
```

Arguments and options as [`sync`](#sync-options), plus `--delete-data`. Scope: **environment**. Admin-tier.
...
**The data buckets need an explicit opt-in.** The env config bucket (the env manifest + env-shared `.env`) and the env logs bucket (ALB access logs) hold data, so they're only emptied and deleted with **`--delete-data`**; without it they're left standing and the plan notes it. Everything else comes down under the normal confirm gate.
