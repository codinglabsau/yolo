...
Runs under the per-app **deployer** tier — the same scoped role that pulls the file at build time — so reading an app's env file requires the admin tier (the deployer role is CI's — an admin assumes it for a laptop run).
...
Runs under the per-app **deployer** tier, whose policy carries get + put on exactly this app's env-file object — so writing an app's secrets requires the admin tier (the deployer role is CI's — an admin assumes it for a laptop run), never a raw-identity S3 grant.
...
Removing a [service](/guide/services#the-service-lifecycle) (`services.{name}`) is refused while a running app still uses it — the error names the app — and likewise while any running app hasn't published what it uses yet. Remove the service from each app's `yolo.yml` and `sync:app` it first; the push goes through once nothing is using it.
...
Claims carry the app's full environment-resolved manifest block (published on every `sync:app`), so an app that hasn't synced since upgrading shows `—` until it republishes.
...
|---|---|---|
| Observer — entire environment | `yolo-{env}-observers` | read every app in the environment and its data |
| Observer — this app only | `yolo-{env}-{app}-observers` | read this app and its data (log content fenced to its log group) |
| Developer — entire environment | `yolo-{env}-developers` | observer + write every app's data and read the database dumps; no infrastructure writes |
| Developer — this app only | `yolo-{env}-{app}-developers` | observer + write this app's data and read its dumps |
| Admin — entire environment | `yolo-{env}-admins` | `sync` / `scale` / `deploy` / manage access |

The deployer role is never offered: it's CI's (GitHub OIDC), and an admin already assumes it for a laptop deploy. Toggling and confirming applies the membership diff — and only ever touches these YOLO groups, never a user's other group memberships. Only groups that have actually been [synced](#yolo-sync) are offered (you can't grant a tier that isn't provisioned). It runs under the **admin** tier, whose policy can manage `yolo-*` group membership, so a member of `yolo-{env}-admins` can grant access to others. To grant a per-app tier on a different app, run it in that app's directory. See [provisioning](/guide/provisioning), and [Developer Credentials](/guide/credentials) for the end-to-end onboarding flow this fits into (IAM user, MFA, local credential setup).
...
Every sync also compares the running CLI's release against the environment's [version-of-record](/guide/provisioning#the-environment-declaration) (the `yolo-version` marker `sync:environment` stamps into the env config bucket). When this checkout is provably older and the plan holds a pending change in the account or environment tier, `sync`, `sync:environment` and `sync:account` **refuse** to apply it (under `--check` too) — an older CLI's checks predate the environment, so it could reconcile newer defaults back without flagging it; update `codinglabsau/yolo` and re-run. A plan clean in those tiers proceeds without even reading the marker. `sync:app` only **warns**, since an app's pinned release routinely lags the environment between releases, and the deploy gate's `--check` passes through to that warning. A `dev-*` pin, an unstamped environment or an unreadable marker can't be ordered and never refuse.
...
Sync a single application's resources for the given environment — S3 buckets, the app's published claim file (`apps/{app}.yml` in the env config bucket), app IAM (deployer role/policy, the per-app **observer** role/policy whose log-content reads are fenced to this app's log group, the per-app **data-read** / **data-write** policies fenced to this app's data bucket and dump prefix (the read policy only when the app declares a [`bucket`](/reference/manifest#bucket)), the per-app **developer** role carrying observer + both data policies, the per-app ECS task role plus any [`task-role-policies`](/reference/manifest#task-role-policies), and the MediaConvert role when the app uses the [`mediaconvert` service](/reference/manifest#services) — torn down again on the sync after the app stops using it), the per-app [grant groups](#conventions) (`yolo-{env}-{app}-observers` and `yolo-{env}-{app}-developers`) whose membership grants read / data-write on this app, ECS cluster/service/task definition, target group + listener rule, CloudFront distribution, SQS queues, a CloudWatch dashboard, target-tracking autoscaling (when configured), and — for a solo app — its hosted zone and ACM certificate. For web apps it also provisions the shared [Valkey cache](/guide/provisioning#cache-and-sessions) (`cache.store`, default-on); sessions ride the same cluster by default ([`session.driver: redis`](/guide/provisioning#cache-and-sessions)), so they need no resources of their own.
