...
Every sync also compares the running CLI's release against the environment's [version-of-record](/guide/provisioning#the-environment-declaration) (the `yolo-version` marker `sync:environment` stamps into the env config bucket). `sync`, `sync:environment` and `sync:account` **refuse** to run when this checkout is provably older — an older CLI's checks predate the environment, so it could reconcile newer defaults back without flagging it; update `codinglabsau/yolo` and re-run. `sync:app` only **warns**, since an app's pinned release routinely lags the environment between releases, and the deploy gate's `--check` passes through to that warning. A `dev-*` pin, an unstamped environment or an unreadable marker can't be ordered and never refuse.
