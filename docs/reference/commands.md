...
Some environment-tier resources are bootstrapped here by exception — the RDS security group (because its real purpose is this app's task-SG ingress), the HTTPS `:443` listener (because its creation needs this app's certificate), the shared Valkey cache when `cache.store` is set (its security group needs this app's task SG to authorise), and the shared Meilisearch service when `scout.driver: meilisearch` is declared (the declaration itself is per-app). All are created-if-missing and never mutated, so the environment tier remains their single writer.
