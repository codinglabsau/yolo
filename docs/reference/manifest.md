...
| `services.typesense` | The environment's [Typesense search cluster](/guide/provisioning#typesense-the-environment-s-search-cluster). `version` (the `typesense/typesense` image tag) is required — an environment never runs an implicit search engine version. `cpu` and `memory` follow the [`tasks.*` conventions](#tasks-web): optional per-node Fargate sizing, defaulting to `'256'`/`'1024'`. `services: { typesense: { version: "29.0" } }` is a complete entry. A version bump or resize is a manifest edit + `sync:environment` — the nodes roll one at a time. |
