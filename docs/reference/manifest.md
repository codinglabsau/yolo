...
| `typesense` | Claims the environment's shared [Typesense search cluster](/guide/provisioning#typesense-the-environment-s-search-cluster) — the claim is what keeps the env-shared cluster provisioned. No runtime IAM (the app talks to the cluster over HTTP with a scoped key). Build-time injection (Scout driver, node addresses, the app's scoped key) ships with the consumption half |
