...
| `typesense` | Claims the environment's shared [Typesense search cluster](/guide/provisioning#typesense-the-environment-s-search-cluster) — the claim is what keeps the env-shared cluster provisioned. No runtime IAM: the app talks to the cluster over HTTP. `sync:app` opens the app's private path (its task SG onto the search API port) and **mints the app a key scoped to its own `{prefix}*` collections**, written into the app's `.env.{environment}`. The build injects `SCOUT_DRIVER=typesense`, `SCOUT_PREFIX`, and the private Cloud Map node addresses (`TYPESENSE_HOST/PORT/PROTOCOL` + the full `TYPESENSE_NODES` list) — server-side indexing rides the VPC, never the ALB/WAF. The app's CloudWatch dashboard gains the search panels |
...
The corresponding env-shared infrastructure is the environment manifest's side of the contract — e.g. the IVS event-logging pipeline (one `/aws/ivs/yolo-{env}` log group + EventBridge rule per environment, because the `aws.ivs` event stream is account-wide) is provisioned by `sync:environment` while `yolo-environment-{environment}.yml` **offers** `services.ivs` *and* a live app claims it — [the two-key service lifecycle](/guide/provisioning#the-service-lifecycle-offered-and-claimed). Claiming an env-backed service the environment doesn't offer is a **hard error** at `build`, `deploy` and `sync:app` (add the offer, or drop the claim); dropping a claim melts the app-side service resources (e.g. the MediaConvert role) on the next sync. Defaulted framework backends ([`cache`](#cache), [`session`](#session)) deliberately stay separate keys — `services` is for opt-in capabilities only.
...
| `services` | The environment's service **offers** — a map of service ⇒ offer config (`services.ivs: {}`). An offer is one of [the two keys](/guide/provisioning#the-service-lifecycle-offered-and-claimed) a service needs to exist: `sync:environment` provisions an offered service while a live app **claims** it ([`services: [ivs]`](#services) in `yolo.yml`) and plans its teardown once none does. Offers deliberately survive zero consumers — the catalogue is the environment's memory of what it runs — and `environment:manifest:push` refuses to remove an offer that live apps still claim. Each offer is a map (never a scalar or list); its allowed keys come from the service's definition. |
| `services.typesense` | The environment's [Typesense search cluster](/guide/provisioning#typesense-the-environment-s-search-cluster). The offer **must** declare its shape — `version` (the `typesense/typesense` image tag), `cpu` and `memory` (per-node Fargate units): `services: { typesense: { version: "29.0", cpu: 256, memory: 1024 } }`. A version bump or resize is a manifest edit + `sync:environment` — the nodes roll one at a time. |
