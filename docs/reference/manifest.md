...
| `services.typesense` | The environment's [Typesense search cluster](/guide/provisioning#typesense-the-environment-s-search-cluster). The entry **must** declare its shape — `version` (the `typesense/typesense` image tag), `cpu` and `memory` (per-node Fargate units): `services: { typesense: { version: "29.0", cpu: 256, memory: 1024 } }`. A version bump or resize is a manifest edit + `sync:environment` — the nodes roll one at a time. |
