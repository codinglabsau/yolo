...
- **`destroy:app <env>`** — tears the manifest's app down (Fargate, storage, app IAM, CDN, DNS) and reverses the per-app slice of any service it consumes — revokes its database-port/cache/Typesense ingress rules, deletes its per-app MediaConvert role, removes its per-app env file (`env/.env.{app}`, which also held minted Typesense keys). The env-shared resources (`:443` listener, cache, search cluster, WAF) stay for other apps. Refuses the shapes whose teardown isn't modelled yet — **multi-tenant**, **headless** (no domain), **no web task**.
