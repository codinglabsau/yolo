...
| `sync:environment <env>` | `Env` | every app in the environment | VPC, subnets, IGW/routes, RDS SG, SNS topic, shared ECS execution role, env S3 bucket (ALB access logs under `alb-logs/`), ALB + `:80` and `:443` listeners |
