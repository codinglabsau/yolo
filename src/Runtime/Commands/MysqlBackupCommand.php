 ...
 *
 * Uploads overwrite `{app}/{database}.sql.zst` in place — the bucket is
 * versioned, so noncurrent versions are the history (swept by lifecycle).
 * The upload uses the task role's write-only grant; the archive is deleted
 * locally either way. A failed database is reported and the run moves on to
...
class MysqlBackupCommand extends Command
{
    ...
    protected function backup(string $database, string $destination): bool
    {
        ...
        try {
            ...
            $this->upload($archive, sprintf('%s/%s.sql.zst', $destination, $database));
