...
/**
 * Only the target section is ever touched — every other byte survives a write
 * verbatim, because these files are hand-maintained and shared with tools YOLO
 * doesn't own. `~/.aws/config` names sections `[profile foo]` (except
 * `[default]`); `~/.aws/credentials` names them `[foo]`.
 */
class SharedIniFile
{
    ...
    /**
     * How the config writer detects SSO remnants that would steer credential
    ...
    public function putSection(string $name, array $lines): void
    {
        ...
        // An in-place replace consumes the old section's trailing blank line.
        if ($tail !== [] && trim($tail[0]) !== '') {
            $section[] = '';
        }
    ...
    public function removeSection(string $name): void
    {
        ...
        // Removing a section between two others leaves both separator blanks at the seam.
