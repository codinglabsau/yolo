...
class StatusPanel implements Panel
{
    ...
    /**
     * Only firing alarms get a row; the full inventory stays one command away via `yolo status:alarms`.
     *
    ...
    /**
     * A raw CloudWatch StateReason can run to hundreds of characters; unbounded it
     * wraps to a second physical row and the fixed-height frame overruns the screen.
     *
     * @param  array{name: string, state: ?string, reason: ?string}  $alarm
     */
    public static function alarmRow(array $alarm, int $width): string
    {
        // 2-space indent + 5-column state badge + a space.
