...
class GenerateSupervisorConfigStep implements Step
{
    ...
    public function __invoke(array $options = []): StepResult
    {
        ...
        // Both serving modes generate into the one Caddyfile path, for different
        // reasons. Octane needs one only when autoscaling: FrankenPHP's gauges are the
        // burst signal, and octane:start runs the file via --caddyfile
        // (ProcessCommands::web). Classic mode always needs one — it is the only channel
        // that can configure FrankenPHP's thread pool at all — and gains the metrics
        // option under the same autoscaling gate, so generation, the burst env and the
        // build preflight (CheckMetricsRuntimeStep) stay in lock-step.
        if (Manifest::hasWeb() && ! Manifest::usesOctane()) {
            $this->writeClassicCaddyfile();
        } elseif (Manifest::usesMetricsCaddyfile()) {
            $this->writeCaddyfile();
        }
     ...
     * — and, when the tier autoscales, the `metrics` global option that exposes the
     * thread gauges burst scaling reads.
     *
    ...
    {
        $caddyfile = Manifest::usesMetricsCaddyfile()
            ? $this->injectMetrics($this->classicCaddyfile())
            : $this->classicCaddyfile();

        $path = Paths::build('docker/Caddyfile');
        $this->filesystem->ensureDirectoryExists(dirname($path));
        $this->filesystem->put($path, $caddyfile);
    }
    ...
    protected function classicCaddyfile(): string
    {
        ...
        // Tab-indented like Caddy's own convention (and Octane's stub), so the metrics
        // line injectMetrics() adds sits flush with the rest of the global block.
        return <<<CADDY
    ...
    /**
     * Add Caddy's top-level `metrics` global option to a Caddyfile — Octane's stub or
     * the generated classic one. This — not the per-server `servers { metrics }` form —
     * is what registers FrankenPHP's gauges (the worker gauges, and
     * frankenphp_busy_threads / _total_threads / _queue_depth) into the global registry
     * the `/metrics` endpoint serves; the per-server form only surfaces caddy_http_* and
