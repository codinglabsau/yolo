...
class GenerateSupervisorConfigStep implements Step
{
    ...
    public function __invoke(array $options = []): StepResult
    {
        ...
        // Both serving modes generate into the one Caddyfile path. Octane needs one only when
        // autoscaling (the gauges are the burst signal, run via --caddyfile); classic mode
        // always needs one — the only channel that can configure FrankenPHP's thread pool —
        // and gains the metrics option under the same autoscaling gate, so generation, the
        // burst env and CheckMetricsRuntimeStep stay in lock-step.
    ...
    /**
     * `php-server` has no way to express thread bounds ({@see WebThreads}); when the tier
     * autoscales the file also carries the `metrics` global option burst reads. YOLO writes
     * it itself so a custom base image can't silently change how the tier serves.
     */
     ...
     * gauges (the burst signal) dark. The global block is the only one opened by a
     * bare `{`.
     */
